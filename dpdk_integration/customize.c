...
void ndpi_analyze_payload(struct ndpi_flow_info *flow,
			  __rte_unused u_int8_t src_to_dst_direction,
			  u_int8_t *payload,
...
__rte_unused static int payload_stats_sort_asc(void *_a, void *_b) {
...
/* ***************************************************** */
#if 0
_void ndpi_report_payload_stats() {
...
static struct ndpi_flow_info *get_ndpi_flow_info(struct ndpi_workflow * workflow,
						 ...
						 __rte_unused u_int16_t ip_offset,
						 u_int16_t ipsize,
...
static struct ndpi_proto packet_processing(struct ndpi_workflow * workflow,
  ...
  if(!flow->detection_completed) {
    ...
    printf("%s()\n", __FUNCTION__);
#endif
    //printf("%s(%d) enough_packets %u\n",__func__,__LINE__, enough_packets);
    if(proto == IPPROTO_TCP)
    ...
    //printf("%s(%d) %u\n",__func__,__LINE__,flow->detected_protocol.app_protocol);
	...
	if(flow->detected_protocol.app_protocol == NDPI_PROTOCOL_UNKNOWN) {
	  ...
	  if(enable_protocol_guess) 
        workflow->stats.guessed_flow_protocols++;
  ...
  if(flow->detected_protocol.app_protocol == NDPI_PROTOCOL_UNKNOWN ||
    flow->detected_protocol.master_protocol == NDPI_PROTOCOL_UNKNOWN ) {
        ...
        flow->detected_protocol.master_protocol = proto;
    }
  *flow_risk = flow->risk;
 ...
 iph_check:
  ...
  v4_warning:
    if(ipv4_warning_used == 0) {
      if(!workflow->prefs.quiet_mode)
        LOG(NDPI_LOG_DEBUG,
		 "\n\nWARNING: only IPv4/IPv6 packets are "
		 supported in this demo (nDPI supports both IPv4 and IPv6), 
		 "all other packets will be discarded\n\n");
      ipv4_warning_used = 1;
    }
    nproto.app_protocol = nproto.master_protocol = NDPI_LAST_IMPLEMENTED_PROTOCOL;
